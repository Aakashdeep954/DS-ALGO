...
void merge(ll arr[], int start, int mid, int end, int &inv_cnt)
{
    int n1 = mid - start + 1;
    int n2 = end - mid;
    ll left[n1], right[n2];
...
void partition(ll arr[], int start, int end, int &inv_cnt)
{
    if (start >= end)
    {
        return;
    ...
    partition(arr, start, mid, inv_cnt);
    partition(arr, mid + 1, end, inv_cnt);
    merge(arr, start, mid, end, inv_cnt);
}
void solve()
{
    int n;
