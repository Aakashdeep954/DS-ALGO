...
void merge(ll arr[], int start, int mid, int end, ll &sum)
{
    int n1 = mid - start + 1;
    int n2 = end - mid;
    ll left[n1], right[n2];
    ...
    while (left_index < n1 && right_index < n2)
    {
        ...
        {
            sum += (left[left_index]) * (n2 - right_index);
...
void partition(ll arr[], int start, int end, ll &sum)
{
    if (start >= end)
    {
        return;
    ...
    partition(arr, start, mid, sum);
    partition(arr, mid + 1, end, sum);
