...
void create_vector_of_nodes(binary_tree<int> *root, vector<int> &nodes)
{
    if (root == NULL)
    {
        return;
    }
    create_vector_of_nodes(root->left, nodes);
    nodes.push_back(root->data);
    create_vector_of_nodes(root->right, nodes);
}

void print_pairs(vector<int> &nodes, int sum)
{
    int start_pointer = 0;
    int end_pointer = nodes.size() - 1;
    while (start_pointer < end_pointer)
    {
        if (nodes[start_pointer] + nodes[end_pointer] < sum)
        {
            start_pointer++;
        }
        else if (nodes[start_pointer] + nodes[end_pointer] > sum)
        {
            end_pointer--;
        }
        else
        {
            cout << nodes[start_pointer] << " " << nodes[end_pointer] << endl;
            start_pointer++;
            end_pointer--;
        }
    }
...
void solve()
{
    ...
    cin >> sum;
    vector<int> nodes;
    create_vector_of_nodes(root, nodes);
    sort(nodes.begin(), nodes.end());
    print_pairs(nodes, sum);
}
int main()
{
