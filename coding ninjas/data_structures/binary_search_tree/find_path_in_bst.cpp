...
vector<int> find_path_in_bst(binary_tree<int> *root, int k)
{
    vector<int> path;
    if (root == NULL)
    {
    ...
    if (root->data > k)
    {
        path = find_path_in_bst(root->left, k);
        if (path.size() != 0)
        {
    ...
    {
        path = find_path_in_bst(root->right, k);
        if (path.size() != 0)
        {
            path.push_back(root->data);
        }
...
void solve()
{
    ...
    int k;
    cin >> k;
    vector<int> output = find_path_in_bst(root, k);
    fo(i, output.size())
    {
        cout << output[i] << " ";
    }
