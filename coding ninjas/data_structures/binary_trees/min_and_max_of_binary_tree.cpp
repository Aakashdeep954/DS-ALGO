...
pair<int, int> get_min_and_max(binary_tree<int> *root)
{
    if (root == NULL)
    {
    ...
    pair<int, int> left = get_min_and_max(root->left);
    pair<int, int> right = get_min_and_max(root->right);
...
void solve()
{
    ...
    pair<int, int> ans = get_min_and_max(root);
