...
pair<int, bool> helper(binary_tree<int> *root)
{
    if (root == NULL)
    {
    ...
    if (left.second == false || right.second == false || abs(left.first - right.first) > 1)
    {
        ans.second = false;
        return ans;
    }
...
bool check_balanced(binary_tree<int>* root){
...
void solve()
{
    ...
    cout << check_balanced(root) ? "true" : "false";
}
int main()
{
