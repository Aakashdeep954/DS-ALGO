...
tree_node<int> *max_data_node(tree_node<int> *root)
{
    if(root==NULL){
        return NULL;
    }
    tree_node<int> *maximum = root;
    fo(i, root->children.size())
    {
        tree_node<int> *max_child = max_data_node(root->children[i]);
        if(maximum->data<max_child->data){
            maximum=max_child;
        }
    }
    return maximum;
...
void solve()
{
    ...
    tree_node<int> *ans = max_data_node(root);
